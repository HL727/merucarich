...
class MercariExcludeSeller(models.Model):
    class Meta:
    ...
    local_id = None
    seller_id = models.CharField(verbose_name="セラーID", max_length=128)
...
class MercariExcludeSellerMaster(models.Model):
    class Meta:
...
# 出品商品
class MercariToAmazonItem(models.Model):
    class Meta:
    ...
    feed_type= models.IntegerField(verbose_name="出品種別",null=True) # 0:新規, 1:相乗り, 3:取り込み, 4: 取り込み途中
    ...
    purchaseo_seller_id= models.CharField(verbose_name="セーラーID(登録時)",max_length=64,null=True, blank=True)
    purchaseo_seller_id_name= models.CharField(verbose_name="セラー名(登録時)",max_length=64,null=True)
    purchase_item_id= models.CharField(verbose_name="商品ID(登録時)",max_length=64,null=True, blank=True)
    purchase_quantity= models.IntegerField(verbose_name="仕入数(登録時)",null=True, blank=True)
    purchase_fulfillment_latency= models.IntegerField(verbose_name="リードタイム(登録時)",null=True, blank=True,db_index=True) 
    purchase_price= models.IntegerField(verbose_name="仕入れ価格(登録時)",null=True, blank=True)
    purchase_similarity= models.FloatField(verbose_name="類似度(登録時)",null=True, blank=True)
    current_purchase_seller_id= models.CharField(verbose_name="セーラーID(現在)",max_length=32,null=True, blank=True,db_index=True)
    current_purchase_seller_id_name= models.CharField(verbose_name="セラー名(現在)",max_length=32,null=True,db_index=True) 
    ...
    amazon_price= models.IntegerField(verbose_name="アマゾン最安値(登録時)",null=True, blank=True)
...
# ファイルを削除する。
@receiver(models.signals.post_delete, sender=MercariToAmazonItem)
def delete_file(sender, instance, **kwargs):    
    base_dir = os.path.join(settings.RICHS_FOLDER_IMAGE, 'mercari', instance.author.username)
    try:
...
# 出品商品
class MercariToAmazonItemCandidate(models.Model):
    class Meta:
    ...
    def delete_file(self):
        ''' ファイルの削除 '''
        base_dir = os.path.join(settings.RICHS_FOLDER_IMAGE, 'mercari', self.author.username)
        try:
...
# CSV出力
class MercariToAmazonCSV(models.Model):
    class Meta:
...
class MercariImportCSVResult(models.Model):
    class Meta:
    ...
    error_mercari_items = models.IntegerField(verbose_name="メルカリ該当商品無し", null=True, blank=True)
    error_mercari_items_txt = models.TextField(verbose_name="メルカリ該当商品無し", null=True, blank=True)
