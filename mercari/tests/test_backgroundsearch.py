...
from mercari.models import MercariToAmazonItem, MercariToAmazonCSV, MercariExcludeSeller

@patch('mercari.views.asynchelpers.wait')
@patch('mercari.views.logger')
@patch('mercari.views.asynchelpers.get_queue')
class MercariBackgroundTaskTests(TestCase):

    def test_entry_if_no_watcher(self, mqueue, mlogger, mwait):
        user = User.objects.create(username='hoge')

        mercari.views.amazon_offer_research_entry(MagicMock(), '', user)
        
        self.assertEquals(mlogger.warn.call_count, 1)
        (actuals, _) = mlogger.warn.call_args_list[0]
        self.assertEquals(actuals[0], 
            '相乗り商品抽出レコードが未登録です。 research_type=1, author=%s')
    ...
    def test_entry_if_mws_is_none(self, mqueue, mlogger, mwait):
        ...
        with patch('mercari.views.RichsUtils.get_mws_api', return_value=None):
            mercari.views.amazon_offer_research_entry(MagicMock(), '', user)

        self.assertEquals(mlogger.info.call_count, 1)
        (actuals, _) = mlogger.info.call_args_list[0]
        self.assertEquals(actuals[0], 
            'MWS APIの情報登録がありません。 research_type=1, author=%s')
    ...
    def _entry(self):
        ...
        params = QueryDict('&'.join([
            'search_type=0&rateing=80.0&similarity=0.8',
            'condition_id_1=1&shipping_payer_id_2=1',
            'status_id_on_sale=1&is_export_csv=1&sort_order=0'
        ]))
        with patch('mercari.views.RichsUtils.get_tmp_image_folder', return_value='/tmp'), \
                patch('mercari.views.RichsUtils.get_mws_api', return_value=MagicMock()), \
                patch('mercari.views.AmazonScraper.get_products', return_value=['mock!']), \
                patch('yahoo.views.timezone.datetime') as mdt:
            mdt.now.return_value = datetime.datetime(2002, 1, 2, 1, 2, 3)
            mercari.views.amazon_offer_research_entry(params, '', user)


    def test_entry_success_production(self, mqueue, mlogger, mwait):
        with patch('mercari.views.settings') as msettings, \
                patch('mercari.views.AmazonScraper.set_mws') as mmws:
        ...
        self.assertEquals(nameless[0], mercari.views.amazon_offer_research_task)
        pools = actuals['pools']
    ...
    def test_entry_raise_unknown_exception(self, mqueue, mlogger, mwait):
        e = Exception('unknown!')
        with patch('mercari.views._get_offer_research_scrapers', side_effect=e):
            self._entry()
        ...
        self.assertEquals(actuals[0], mercari.views.amazon_offer_research_finalize)
    ...
    def test_entry_retry_when_amazon_response_5xx(self, mqueue, mlogger, mwait):
        ...
        with patch('mercari.views.RichsUtils.get_tmp_image_folder', return_value='/tmp'), \
                patch('mercari.views.RichsUtils.get_mws_api', return_value=MagicMock()), \
                patch('mercari.views._amazon_retry_settings', return_value=retry_settings), \
                patch('mercari.views.AmazonScraper.get_products', side_effect=ValueError('503')):
            mercari.views.amazon_offer_research_entry(params, '', user)

        user = User.objects.get(username='hoge')
        (actuals, kwactuals) = mqueue().enqueue.call_args_list[0]
        kwargs = kwactuals['kwargs']
        self.assertEquals(actuals[0], mercari.views.amazon_offer_research_entry)
        ...
        with patch('mercari.views.RichsUtils.get_tmp_image_folder', return_value='/tmp'), \
                ...
                patch('mercari.views._amazon_retry_settings', return_value=retry_settings), \
                patch('mercari.views.AmazonScraper.get_products', side_effect=ValueError('503')):
            mercari.views.amazon_offer_research_entry(params, '', user, trial=33, rest_wait_count=1)
        ...
        self.assertEquals(kwargs['url'], '')
        self.assertEquals(kwargs['user'], user)
        self.assertEquals(kwargs['trial'], 33)
        ...
        with patch('mercari.views.RichsUtils.get_tmp_image_folder', return_value='/tmp'), \
                ...
                patch('mercari.views._amazon_retry_settings', return_value=retry_settings), \
                patch('mercari.views.AmazonScraper.get_products', side_effect=ValueError('503')):
            mercari.views.amazon_offer_research_entry(params, '', user, trial=120)
        user = User.objects.get(username='hoge')
        mlogger.info.assert_called_once_with(
            'ユーザー %s の処理中にエラーが発生したため、強制中断しました', user)
        (actuals, _) = mqueue().enqueue.call_args_list[0]
    ...
    def test_maintask_for_urlskip_requests(self, mqueue, mlogger, mwait):
        ...
        user = User.objects.get(username='hoge')
        URLSkipRequest.objects.create(author=user, view=21)

        # ヤフオク検索のOfferReserchWatcher を追加
        OfferReserchWatcher.objects.create(
            research_type = 0, status = 0, total = 0, exclude_asin = 0, 
            prime = 0, condition_different = 0, exclude_seller = 0, not_found = 0, 
            feed_item = 0, is_over_items=False, author = user)

        # メルカリ検索の Watcher は存在する
        self.assertEquals(
            OfferReserchWatcher.objects.filter(author=user, research_type=1).count(), 1)
        
        # テスト対象呼び出し
        mercari.views.amazon_offer_research_task(args['params'], '', user, args['pools'])
        ...
        self.assertEquals(URLSkipRequest.objects.filter(view=21, author=user).count(), 0)
        self.assertEquals(
    ...
    def test_maintask_for_stop_requests(self, mqueue, mlogger, mwait):
        args = self._entry_for_maintask(mqueue)
        user = User.objects.get(username='hoge')
        StopRequest.objects.create(author=user, view=21)
        ...
        self.assertEquals(StopRequest.objects.filter(view=21, author=user).count(), 0)
        self.assertEquals(
            OfferReserchWatcher.objects.filter(author=user, research_type=0).count(), 1)
        self.assertEquals(
            OfferReserchWatcher.objects.filter(author=user, research_type=1).count(), 0)
    ...
    def test_maintask_for_empty_input(self, mqueue, mlogger, mwait):
        ...
        with patch('mercari.views.AmazonScraper', amazon_scraper):
            mercari.views.amazon_offer_research_task(args['params'], '', user, args['pools'])
        ...
        # next_url なしの旧式リクエストの場合は値を別途取得する
    ...
    def test_maintask_raised_exception_in_item_research(self, mqueue, mlogger, mwait):
        ...
        with patch('mercari.views.AmazonScraper', amazon_scraper), \
                patch('mercari.views.amazon_offer_research_for_item',  
                    side_effect=Exception('Mock Exception')) as m:
            mercari.views.amazon_offer_research_task(
    ...
    def test_maintask_over_item(self, mqueue, mlogger, mwait):
        ...
        with patch('mercari.views.AmazonScraper', amazon_scraper), \
                patch('mercari.views.amazon_offer_research_for_item', side_effect=fake):
            mercari.views.amazon_offer_research_task(
                args['params'], 'url', user, pools)
    ...
    def test_maintask_can_break_when_time_over(self, mqueue, mlogger, mwait):
        ...
        with patch('mercari.views.datetime') as mdt, \
                patch('mercari.views.AmazonScraper', amazon_scraper), \
                patch('mercari.views.amazon_offer_research_for_item', side_effect=fake):
            # Itemを2個検索する
    ...
    def test_maintask_has_next_url(self, mqueue, mlogger, mwait):
        ...
        self.assertEquals(actuals[0], mercari.views.amazon_offer_research_task)
        self.assertEquals(kwactuals['url'], 'initial_next_url')
        self.assertEquals(kwactuals['pools']['sequence'], 1)
        self.assertEquals(kwactuals['pools']['next_url'], 'next_url')
        self.assertEquals(kwactuals['pools']['amazon_items'], ['dummy1'])
        self.assertEquals(kwactuals['pools']['rest_amazon_items'], ['dummy1'])
    ...
    def test_maintask_raise_amazon_response_5xx(self, mqueue, mlogger, mwait):
        ...
        with patch('mercari.views.AmazonScraper', amazon_scraper), \
                ...
                patch('mercari.views.amazon_offer_research_for_item'):
            mercari.views.amazon_offer_research_task(
                args['params'], 'url', user, pools, trial=22, rest_wait_count=1)
        ...
        self.assertEquals(kwargs['user'], user)
        self.assertEquals(kwargs['pools'], pools)
        # rest_wait_count のカウントのみ 1 下がる
        self.assertEquals(kwargs['trial'], 22)
        self.assertEquals(kwargs['rest_wait_count'], 0)
    ...
    def test_maintask_has_next_url_and_reaches_maximum(self, mqueue, mlogger, mwait):
        ...
        with patch('mercari.views.asynchelpers.reached_maximum_sequence', return_value=True), \
                patch('mercari.views.AmazonScraper', amazon_scraper), \
                patch('mercari.views.amazon_offer_research_for_item'):
            mercari.views.amazon_offer_research_task(
                args['params'], 'url', user, pools)
    ...
    def test_maintask_raise_unknown_exception(self, mqueue, mlogger, mwait):
        ...
        with patch('mercari.views.datetime', MagicMock()) as mdt:
    ...
    def test_itemtask_for_exclude_asin(self, mqueue, mlogger, mwait):
        ...
        mercari.views.amazon_offer_research_for_item(
    ...
    def test_itemtask_for_price_mercari_item_not_found(self, mqueue, mlogger, mwait):
        args = self._entry_for_maintask(mqueue)
        user = User.objects.get(username='hoge')
        watch = OfferReserchWatcher.objects.get(author=user)
        ExcludeAsin.objects.create(author=user, asin='X123456789')
        ...
        mercari_scraper = MagicMock()
        mercari_scraper().get_products = MagicMock(return_value=[])

        self.assertEquals(watch.not_found, 0)
        with patch('mercari.views.MercariSearchScraper', mercari_scraper):
            mercari.views.amazon_offer_research_for_item(
                args['params'], user, pools, watch, amazon_item)
    ...
    def test_itemtask_for_price_mercari_item_all_invalid(self, mqueue, mlogger, mwait):
        args = self._entry_for_maintask(mqueue)
        user = User.objects.get(username='hoge')
        watch = OfferReserchWatcher.objects.get(author=user)
        ...
        mercari_items = [
            { 'title': 'banned title', 'item_id': 'W123456789', 'images': ['image/mercari3'], },
            { 'title': 'title', 'item_id': 'Y123456789', 'images': ['image/mercari'], },
            { 'title': 'title', 'item_id': 'Z123456789', 'images': ['image/mercari2'], },
        ]
        mercari_scraper = MagicMock()
        mercari_scraper().get_products = MagicMock(return_value=mercari_items)
        mercari_scraper().save_image = MagicMock(return_value='mock/mercari')

        mdownloader = MagicMock()
        mdownloader().__enter__().get = MagicMock(return_value='/tmp/mock/mercari')
        
        self.assertEquals(watch.condition_different, 0)
        with patch('mercari.views.ItemImageComparator.similar_fast', return_value=(False, 0.0)) as msimilar, \
                patch('mercari.views.AmazonScraper', amazon_scraper), \
                patch('mercari.views.MercariSearchScraper', mercari_scraper), \
                patch('mercari.views.ImageDownloader', mdownloader), \
                patch('mercari.views.RichsUtils.delete_file_if_exist'):
            mercari.views.amazon_offer_research_for_item(
                args['params'], user, pools, watch, amazon_item)
            # downloader.get の戻り値が第2引数
            self.assertEquals(msimilar.call_count, 2)
            (args, _) = msimilar.call_args_list[0]
            self.assertEquals(args[0], '/tmp/mock/amazon')
            self.assertEquals(args[1], '/tmp/mock/mercari')
            (args, _) = msimilar.call_args_list[1]
            self.assertEquals(args[0], '/tmp/mock/amazon')
            self.assertEquals(args[1], '/tmp/mock/mercari')
    ...
    def test_itemtask_for_price_mercari_item_skipped(self, mqueue, mlogger, mwait):
        args = self._entry_for_maintask(mqueue)
        user = User.objects.get(username='hoge')
        watch = OfferReserchWatcher.objects.get(author=user)
        ExcludeAsin.objects.create(author=user, asin='X123456789')
        ...
        mercari_scraper().get_products = MagicMock(return_value=mercari_items)
        mercari_scraper().save_image = MagicMock(return_value='mock/mercari')
        mercari_item_scraper = MagicMock()
        mercari_item_scraper().get_products = MagicMock(side_effect=[
            [{'seller': 'bad.seller', 'rate_percent': '1.0' }], 
            [{'seller': 'no.good.seller', 'rate_percent': '0.79' }], 
        ])

        mdownloader = MagicMock()
        mdownloader().__enter__().get = MagicMock(return_value='/tmp/mock/mercari')

        with patch('mercari.views.ItemImageComparator.similar_fast', return_value=(True, 1.0)), \
                patch('mercari.views.AmazonScraper', amazon_scraper), \
                patch('mercari.views.MercariSearchScraper', mercari_scraper), \
                patch('mercari.views.MercariItemScraper', mercari_item_scraper), \
                patch('mercari.views.ImageDownloader', mdownloader), \
                patch('mercari.views.RichsUtils.delete_file_if_exist'), \
                patch('mercari.views.RichsUtils.is_exclude_sellers', side_effect=[True, False]):
    ...
    def test_itemtask_for_price_mercari_item_comparas(self, mqueue, mlogger, mwait):
        args = self._entry_for_maintask(mqueue)
        user = User.objects.get(username='hoge')
        watch = OfferReserchWatcher.objects.get(author=user)
        ExcludeAsin.objects.create(author=user, asin='X123456789')
        ...
        mercari_scraper().save_image = MagicMock(return_value='mock/mercari')
        mercari_item_scraper = MagicMock()
        mercari_item_scraper().get_products = MagicMock(return_value=[
            {'seller': 'good.seller', 'rate_percent': '100.0',
              'bid_or_buy': '3000', 'current_price': '2000', 'seller': 'Y1',
              'condition': '新品', 'images': ['image/mercari1', 'image/mercari2'] }, 
        ])
        ...
        with patch('mercari.views.ItemImageComparator.similar_fast', return_value=(True, 1.0)), \
            ...
            with patch('mercari.views.available_for_purchase', return_value=-3):
            ...
            with patch('mercari.views.available_for_purchase', return_value=-1):
                self.assertEquals(watch.feed_item, 0)
                self.assertFalse(watch.is_over_items)
                self.assertEquals(watch.exclude_seller, 0)
                self.assertEquals(watch.condition_different, 0)
            ...
            with patch('mercari.views.available_for_purchase', return_value=0):
                # 最終画像判定が NG
                dummy1 = MagicMock()
                ...
                with patch('mercari.views.ItemImageComparator.similar', return_value=(False, [])), \
                      patch('mercari.views.print'), \
                      patch('mercari.views.create_item_candidate_in_ride_search', side_effect=[dummy1, dummy2]):
                    self.assertEquals(watch.feed_item, 0)
                    self.assertFalse(watch.is_over_items)
                    self.assertEquals(watch.exclude_seller, 0)
                    self.assertEquals(watch.condition_different, 0)
                ...
                with patch('mercari.views.ItemImageComparator.similar', return_value=(True, [])), \
                        patch('mercari.views.store', return_value=-1):
                    self.assertEquals(watch.feed_item, 0)
                    self.assertFalse(watch.is_over_items)
                    self.assertEquals(watch.exclude_seller, 0)
                    self.assertEquals(watch.condition_different, 0)
                ...
                # 全てOK
                with patch('mercari.views.ItemImageComparator.similar', return_value=(True, [])), \
                        patch('mercari.views.store', return_value=0):
                    self.assertEquals(watch.feed_item, 0)
                    self.assertFalse(watch.is_over_items)
                    self.assertEquals(watch.exclude_seller, 0)
                    self.assertEquals(watch.condition_different, 0)
    ...
    def test_finalizetask_raise_exception(self, mqueue, mlogger, mwait):
        ...
        mercari.views.amazon_offer_research_finalize(args['params'], user)
    ...
    def test_finalizetask_without_csv_download(self, mqueue, mlogger, mwait):
        ...
        StopRequest.objects.create(view=21, author=user)
        self.assertEquals(StopRequest.objects.filter(view=21, author=user).count(), 1)

        with patch('mercari.views.datetime') as mdt:
            mdt.now = MagicMock(return_value=datetime.datetime(2000, 1, 1, 0, 0, 0))
            params = QueryDict('is_export_csv=0')
            mercari.views.amazon_offer_research_finalize(params, user)
    ...
    def test_finalizetask_with_csv_download_and_no_item(self, mqueue, mlogger, mwait):
        ...
        MercariToAmazonItem.objects.filter(feed_type=1, csv_flag=0, author=user).delete()
        self.assertEquals(MercariToAmazonItem.objects.filter(feed_type=1, csv_flag=0, author=user).count(), 0)

        with patch('mercari.views.datetime') as mdt, \
                patch('mercari.views.export_amazon_offer_csv_internal') as minternal:
            mdt.now = MagicMock(return_value=datetime.datetime(2000, 1, 1, 0, 0, 0))
            params = QueryDict('is_export_csv=1')
            mercari.views.amazon_offer_research_finalize(params, user)
            # MercariToAmazonItem がない場合は呼び出しが発生しない
    ...
    def test_finalizetask_with_csv_download_and_item(self, mqueue, mlogger, mwait):
        ...
        self.assertEquals(
            MercariToAmazonItem.objects.filter(feed_type=1, csv_flag=0, author=user).count(), 1)
        self.assertEquals(
            MercariToAmazonItem.objects.filter(feed_type=1, csv_flag=1, author=user).count(), 0)
        self.assertEquals(
            MercariToAmazonCSV.objects.filter(feed_type=1, author=user).count(), 0)

        with patch('mercari.views.print'), \
                patch('mercari.views.datetime') as mdt, \
                patch('mercari.views.os.path.isdir', return_value=True), \
                patch('mercari.views.codecs'), \
                patch('mercari.views.settings') as msettings:
        ...
        # updated
        self.assertEquals(
            MercariToAmazonItem.objects.filter(feed_type=1, csv_flag=0, author=user).count(), 0)
        self.assertEquals(
            MercariToAmazonItem.objects.filter(feed_type=1, csv_flag=1, author=user).count(), 1)

        # added
        self.assertEquals(
            MercariToAmazonCSV.objects.filter(feed_type=1, author=user).count(), 1)
    ...
    def test_finalizetask_with_csv_download_exception(self, mqueue, mlogger, mwait):
        ...
        with patch('mercari.views.print'), \
                patch('mercari.views.datetime') as mdt, \
                patch('mercari.views.export_amazon_offer_csv_internal', side_effect=Exception('csv error')):
            mdt.now = MagicMock(return_value=datetime.datetime(2000, 1, 1, 0, 0, 0))
            params = QueryDict('is_export_csv=1')
            mercari.views.amazon_offer_research_finalize(params, user)
    ...
    def test_available_for_purchase(self, mqueue, mlogger, mwait):
        ''' views.available_for_purchase の C0 カバレッジ '''
        # デフォルト設定の場合
        ...
        mercari_item = {
            'title': '【送料無料・激レア】メルカリの有効なアイテムは30文字以上。', 
            'item_id': 'SKU',
            'condition': 'DUMMY',
            'price': '4200',
        ...
        with patch('mercari.views.settings') as msettings:
            ...
            msettings.MERCARI_RIDE_SEARCH_CONFIG = ridesearch_conf

            with patch('mercari.views.MercariToAmazonItem.objects.filter') as mf:
                mf().count.return_value = 11
                code = mercari.views.available_for_purchase(
                    amazon_item, mercari_item, value, user, price_setting,
                    amazon_image)
            ...
            with patch('mercari.views.MercariToAmazonItem.objects.filter') as mf, \
                    patch('mercari.views.RichsUtils.mercari_to_amazon_condition', return_value='ERR'):
            ...
            with patch('mercari.views.MercariToAmazonItem.objects.filter') as mf, \
                    patch('mercari.views.RichsUtils.mercari_to_amazon_condition', return_value='New'):
                mf().count.return_value = 0
                dummy_item = {'price_new': '-1', 'price_old': '-1'}
                code = mercari.views.available_for_purchase(
                    dummy_item, mercari_item, value, user, price_setting,
                    amazon_image)
                self.assertEqual(code, -1)

            with patch('mercari.views.MercariToAmazonItem.objects.filter') as mf, \
                    patch('mercari.views.RichsUtils.mercari_to_amazon_condition', return_value='Old'):
                mf().count.return_value = 0
                dummy_item = {'price_new': '-1', 'price_old': '-1'}
                code = mercari.views.available_for_purchase(
                    dummy_item, mercari_item, value, user, price_setting,
            ...
            with patch('mercari.views.MercariToAmazonItem.objects.filter') as mf, \
                ...
                code = mercari.views.available_for_purchase(
                    amazon_item, mercari_item, value, user, dummy,
                    amazon_image)
                self.assertEqual(code, -1)
            ...
            with patch('mercari.views.MercariToAmazonItem.objects.filter') as mf, \
                    patch('mercari.views.RichsUtils.mercari_to_amazon_condition', return_value='New'):
                mf().count.return_value = 0
                dummy_item = mercari_item.copy()
                dummy_item['title'] = '【送料無料・激レア】メルカリの有効なアイテムは30文字以上'
                code = mercari.views.available_for_purchase(
    ...
    def test_store_succeeded(self, mqueue, mlogger, mwait):
        ''' views.available_for_purchase の C0 カバレッジ '''
        amazon_item = {
            'title': 'amazon title',
            'seller': 'good.seller',
        ...
        price_setting.offset_offer_price_url = -4000
        price_setting.margin_offer_url = 1
        amazon_default_setting = MagicMock()
        amazon_default_setting.condition_note = 'HOGE'
        amazon_default_setting.fulfillment_latency = 99
        amazon_image = '/tmp/mock/amazon'

        with patch('mercari.views.print'), \
                patch('mercari.views.MercariToAmazonItem') as m2a_item, \
                patch('mercari.views.RichsUtils.mercari_to_amazon_condition', return_value='New'), \
                patch('mercari.views.RichsUtils.copy_image_to_mercari_folder', return_value='/path/to'):
            dummy = MagicMock()
            m2a_item.return_value = dummy
            m2a_item.objects.filter().count.return_value = 0
            code = mercari.views.store(
                amazon_item, mercari_item, value, user, price_setting,
                amazon_default_setting, amazon_image)
            self.assertEqual(code, 0)
            self.assertEqual(dummy.condition_note, 'HOGE')
            self.assertEqual(dummy.fulfillment_latency, 99)
    ...
    def test_item_list_status_message(self, mqueue, mlogger, mwait):
        ...
        item = MercariToAmazonItem()
        item.update_quantity_request = 0
        item.current_purchase_quantity = 0
        self.assertEqual('取り下げ完了', mercari.views._item_list_status_message(item)) 
        item.update_quantity_request = 1
        item.current_purchase_quantity = 0
        self.assertEqual('取り下げ中(適用待ち)', mercari.views._item_list_status_message(item)) 
        self.assertEqual('取り下げ中<br>(適用待ち)', mercari.views._item_list_status_message(item, br=True)) 
        item.update_quantity_request = 0
        item.current_purchase_quantity = 1
        self.assertEqual('出品中', mercari.views._item_list_status_message(item)) 
        item.update_quantity_request = 1
        item.current_purchase_quantity = 1
        self.assertEqual('出品中(適用待ち)', mercari.views._item_list_status_message(item)) 
        self.assertEqual('出品中<br>(適用待ち)', mercari.views._item_list_status_message(item, br=True)) 
    ...
    def test_item_list_inventory_message(self, mqueue, mlogger, mwait):
        ...
        self.assertEqual('在庫数適用待ち(リードタイム適用待ち)', 
            mercari.views._item_list_inventory_message(item)) 
        self.assertEqual('在庫数適用待ち<br>リードタイム適用待ち', 
            mercari.views._item_list_inventory_message(item, br=True)) 
        item.update_quantity_request = 1
        item.update_fulfillment_latency_request = 0
        self.assertEqual('在庫数適用待ち', mercari.views._item_list_inventory_message(item)) 
        ...
        item.update_fulfillment_latency_request = 1
        item.record_type = 0
        self.assertEqual('リードタイム適用待ち', mercari.views._item_list_inventory_message(item)) 

        item.update_quantity_request = 0
        item.update_fulfillment_latency_request = 0
        item.record_type = 0
        self.assertEqual('', mercari.views._item_list_inventory_message(item)) 
    ...
    def test_url_search_cancel(self, mqueue, mlogger, mwait):
        ...
        with patch('mercari.views.datetime') as mdt, \
                patch('mercari.views._ride_search_config', return_value=ride_search_config), \
                patch('mercari.views.amazon_offer_research_for_item', side_effect=not_found):
            mdt.now = MagicMock(return_value=now)
            mercari.views.amazon_offer_research_task(args['params'], 'url', user, pools)
        ...
        self.assertEquals(nameless[0], mercari.views.amazon_offer_research_finalize)
        self.assertEquals(mlogger.info.call_count, 1)
        self.assertEquals(mlogger.info.call_args[0][0], 
    ...
    def test_url_search_found_but_cancel(self, mqueue, mlogger, mwait):
        ...
        with patch('mercari.views.datetime') as mdt, \
                patch('mercari.views._ride_search_config', return_value=ride_search_config), \
                patch('mercari.views.amazon_offer_research_for_item', side_effect=lower_found):
            mdt.now = MagicMock(return_value=now)
            mercari.views.amazon_offer_research_task(args['params'], 'url', user, pools)
    ...
    def test_url_search_found_and_next_search(self, mqueue, mlogger, mwait):
        ...
        with patch('mercari.views.datetime') as mdt, \
                patch('mercari.views._ride_search_config', return_value=ride_search_config), \
                patch('mercari.views.AmazonScraper.get_products', return_value=['mock!']), \
                patch('mercari.views.AmazonScraper.get_next_page_url', return_value='mock_next_url'), \
                patch('mercari.views.amazon_offer_research_for_item', side_effect=found):
            mdt.now = MagicMock(return_value=now)
            mercari.views.amazon_offer_research_task(args['params'], 'url', user, pools)
