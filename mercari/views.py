...
from yahoo.forms import AmazonSearchForm
...
import cv2
import inspect
from accounts.models import OfferReserchWatcher, StopRequest, URLSkipRequest, \
    BackgroundSearchInfo, ItemCandidateToCsv, OverrideConstantValue, FJCMember
import csv
import codecs
import re
import os
...
from scraper import AmazonScraper, MercariSearchScraper, MercariItemScraper, ImageDownloader
from .forms import MercariSearchForm, AmazonOfferMercariSearchForm, MercariExcludeSellerForm, FeedNewForm, FeedOfferForm, MercariItemListSearchForm
from .models import MercariExcludeSeller, MercariToAmazonItem, MercariToAmazonItemCandidate, MercariToAmazonCSV, MercariImportCSVResult
from richs_utils import (
    RichsUtils, ItemImageComparator, ResponseUtils,
    ItemTitleConverter, Functional, AmazonSearchUtils
)
...
def item_list_for_search(form, user, paging_ts, offset, limit):
    ...
    if keyword == '' or (t == '4' and keyword.isdecimal() == False):
        items = MercariToAmazonItem.objects.filter(
            author=user, csv_flag=1, updated_date__lte=paging_ts).order_by('-updated_date')[offset:limit]
        c = MercariToAmazonItem.objects.filter(
            author=user, csv_flag=1, updated_date__lte=paging_ts).count()
        return items, c
    ...
    if search_condition == '0':
        ...
        if t == '0':
            items = MercariToAmazonItem.objects.filter(
                author=user, item_name=keyword, csv_flag=1, updated_date__lte=paging_ts).order_by('-updated_date')[offset:limit]
            c = MercariToAmazonItem.objects.filter(
                author=user, item_name=keyword, csv_flag=1, updated_date__lte=paging_ts).count()
            return items, c

        # Amazon SKU
        elif t == '1':
            items = MercariToAmazonItem.objects.filter(
                author=user, item_sku=keyword, csv_flag=1, updated_date__lte=paging_ts).order_by('-updated_date')[offset:limit]
            c = MercariToAmazonItem.objects.filter(
                author=user, item_sku=keyword, csv_flag=1, updated_date__lte=paging_ts).count()
            return items, c

        # 仕入れ元オークションID
        elif t == '2':
            items = MercariToAmazonItem.objects.filter(
                author=user, current_purchase_item_id=keyword, csv_flag=1, updated_date__lte=paging_ts).order_by('-updated_date')[offset:limit]
            c = MercariToAmazonItem.objects.filter(
                author=user, current_purchase_item_id=keyword, csv_flag=1, updated_date__lte=paging_ts).count()
            return items, c
        # 出品者ID
        elif t == '3':
            items = MercariToAmazonItem.objects.filter(
                author=user, current_purchase_seller_id=keyword, csv_flag=1, updated_date__lte=paging_ts).order_by('-updated_date')[offset:limit]
            c = MercariToAmazonItem.objects.filter(
                author=user, current_purchase_seller_id=keyword, csv_flag=1, updated_date__lte=paging_ts).count()
            return items, c
        # リードタイム
        elif t == '4':
            items = MercariToAmazonItem.objects.filter(author=user, fulfillment_latency=int(
                keyword), csv_flag=1, updated_date__lte=paging_ts).order_by('-updated_date')[offset:limit]
            c = MercariToAmazonItem.objects.filter(author=user, fulfillment_latency=int(
                keyword), csv_flag=1, updated_date__lte=paging_ts).count()
            return items, c
    ...
    # 部分一致
        ...
        if t == '0':
            items = MercariToAmazonItem.objects.filter(
                author=user, item_name__contains=keyword, csv_flag=1, updated_date__lte=paging_ts).order_by('-updated_date')[offset:limit]
            c = MercariToAmazonItem.objects.filter(
                author=user, item_name__contains=keyword, csv_flag=1, updated_date__lte=paging_ts).count()
            return items, c
        # Amazon SKU
        elif t == '1':
            items = MercariToAmazonItem.objects.filter(
                author=user, item_sku__contains=keyword, csv_flag=1, updated_date__lte=paging_ts).order_by('-updated_date')[offset:limit]
            c = MercariToAmazonItem.objects.filter(
                author=user, item_sku__contains=keyword, csv_flag=1, updated_date__lte=paging_ts).count()
            return items, c
        # 仕入れ元オークションID
        elif t == '2':
            items = MercariToAmazonItem.objects.filter(
                author=user, current_purchase_item_id__contains=keyword, csv_flag=1, updated_date__lte=paging_ts).order_by('-updated_date')[offset:limit]
            c = MercariToAmazonItem.objects.filter(
                author=user, current_purchase_item_id__contains=keyword, csv_flag=1, updated_date__lte=paging_ts).count()
            return items, c
        # 出品者ID
        elif t == '3':
            items = MercariToAmazonItem.objects.filter(
                author=user, current_purchase_seller_id__contains=keyword, csv_flag=1, updated_date__lte=paging_ts).order_by('-updated_date')[offset:limit]
            c = MercariToAmazonItem.objects.filter(
                author=user, current_purchase_seller_id__contains=keyword, csv_flag=1, updated_date__lte=paging_ts).count()
            return items, c
        # リードタイム
        elif t == '4':
            items = MercariToAmazonItem.objects.filter(author=user, fulfillment_latency=int(
...
def _item_list_inventory_message(item, br=False):
    ...
    if item.record_type in [20, 21]:
        if br:
            ...
            msg += '<br>在庫数を「{0}」に変更しました。'.format(
                item.current_purchase_quantity)
...
def convert_item_list_to_csv_rows(items):
    ...
    for (idx, item) in enumerate(items):
        rows.append([
            ...
            'https://item.mercari.com/jp/{}'.format(
                item.current_purchase_item_id),
            'https://www.mercari.com/jp/u/{}'.format(
                item.current_purchase_seller_id),
...
def item_list(request):
    ...
    if (request.method == 'POST'):
        # 元画面のフォーム
        form = MercariItemListSearchForm(request.POST)
        if form.is_valid():
            # 新規検索
            ...
            if is_delete:
                ...
                MercariToAmazonItem.objects.filter(
                    id__in=ids, author=request.user).delete()
                params['message'] = "削除完了"
                params['message_detail'] = "削除が完了しました。"
            # SKUを指定して削除
            is_delete_skus = True if 'delete_skus' in request.POST else False
            if is_delete_skus:
                skus_list = RichsUtils.keyslist_to_array(
                    request.POST.get('delete_skus_text'))
                if (len(skus_list) > 0):
                    MercariToAmazonItem.objects.filter(
                        author=request.user, item_sku__in=skus_list).delete()
                    params['message'] = "削除完了"
                    params['message_detail'] = "削除が完了しました。"
                else:
                    params['message'] = "失敗"
            ...
            if export_csv:
                ...
                (items, _count) = item_list_for_search(
                    form, request.user, ts, 0, None)
                filename = 'itemlist_{}.csv'.format(
            ...
            if is_search:
                result = item_list_for_search(
                    form, request.user, ts, offset, limit)
                items = result[0]
            ...
            if (items == None and (is_prev or is_next)):
                items = MercariToAmazonItem.objects.filter(
                    author=request.user, csv_flag=1, updated_date__lte=ts).order_by('-updated_date')[offset:limit]
                items_count = MercariToAmazonItem.objects.filter(
                    author=request.user, csv_flag=1, updated_date__lte=ts).count()
    ...
    if items == None:
        items = MercariToAmazonItem.objects.filter(
            author=request.user, csv_flag=1).order_by('-updated_date')[offset:limit]
        items_count = MercariToAmazonItem.objects.filter(
            author=request.user, csv_flag=1).count()
    ...
    params['feed_items_count'] = MercariToAmazonItem.objects.filter(
    ...
    for i, item in enumerate(items):
        ...
        if (item.main_image_url.startswith('http') == False):
            item.main_image_url = RichsUtils.get_mercari_image_url(
                request.user, item.main_image_url)
    ...
    if form == None:
        form = MercariItemListSearchForm()

    # タイムスタンプ情報を埋め込む
    params['page'] = page
    params['items'] = items
    params['form'] = form
    return render(request, 'mercari/item_list.html', params)


# DBにCSV情報を取り込み
def import_item_internal_csv_to_db(path, user):
    ...
    # TODO
    remain = user.max_items - \
        MercariToAmazonItem.objects.filter(author=user).count()
    ...
    with open(path, 'r', encoding='cp932') as f:
        for i, row in enumerate(csv.reader(f, delimiter="\t")):
            try:
                ...
                # データ設定有無チェック
                row[0] = row[0].strip()  # ItemID
                row[1] = row[1].strip()  # ItemSKU on Amazon (基本ItemIDと同じ)
                row[2] = row[2].strip()  # Amazon ASIN
                ...
                # 除外ASIN
                if RichsUtils.is_exclude_asins(exclude_asins, row[2]):
                    error_record_numbers.append(
                        '{} (除外ASIN<{}>)'.format(str(i+1), row[2]))
                    continue

                # 既存情報を上書き可能かをチェック
                items = MercariToAmazonItem.objects.filter(
                    Q(author=user) & Q(feed_type=3) & Q(csv_flag=1) & Q(item_sku=row[1]) & (
                        Q(item_sku=row[0]) | Q(purchase_item_id=row[0]) | Q(
                            current_purchase_item_id=row[0])
                    ))
                if len(items) > 0:
                    ...
                    updated.external_product_id = row[2]
                ...
                count = MercariToAmazonItem.objects.filter(
                    Q(author=user) & (Q(item_sku=row[0]) | Q(current_purchase_item_id=row[0]) | Q(item_sku=row[1]) | Q(current_purchase_item_id=row[1]))).count()
                if (count > 0):
                    print('重複レコード検出')
                    duplicate_skus.append(row[1])
                    continue

                # 登録件数オーバ
                if remain < 1:
                    over_skus.append(row[1])
                    continue

                # CSVデータをインポートフラグを立ててDBに取り込む
                e = MercariToAmazonItem()
                e.current_purchase_item_id = e.purchase_item_id = row[0]
                e.item_sku = row[1]
                e.external_product_id = row[2]
                e.feed_type = 3
...
# テンポラリーデータを削除
def import_item_internal_delete_tmpdata(user):
    # 更新中データに関しては更新失敗として0件で残す(取り下げがあるため、レコードは消せない)
    update_failed_items = MercariToAmazonItem.objects.filter(
    ...
    # それ以外の中断データは消去する
    MercariToAmazonItem.objects.filter(
        author=user, record_type__in=[10, 11, 12, 13]).delete()


def import_item_cancel_by_exception(result, user, err):
    ''' CSV取り込みの中断処理 '''
    # 予期せぬ例外が起こった場合
...
def import_item_entry(path, user):
    ''' CSV取り込みを実行する遅延タスクのエントリーポイント '''
    # データの事前準備
    result = MercariImportCSVResult.objects.filter(
        author=user, status=1).first()
    if result is None:
    ...
    try:
        # 不要ファイルの削除
        import_item_internal_delete_tmpdata(user)

        # CSVファイルの取り込み
        try:
            (error_record_numbers, duplicate_skus,
             over_skus) = import_item_internal_csv_to_db(path, user)
            result.error_record_numbers = len(error_record_numbers)
            result.error_record_numbers_txt = '\n'.join(
                map(str, error_record_numbers))
        ...
        q.enqueue(import_item_internal_mercari_task, user=user)

    except Exception as err:
        # 想定しない例外の発生時は強制キャンセル
        import_item_cancel_by_exception(result, user, err)
...
def import_item_internal_mercari_task(user, latest_id=-1):
    ''' メルカリ出品情報を取得してDBに登録する継続タスク。
    latest_id は再開時の MercariToAmazonItem の id となる。 '''
    started = datetime.now()
    worktime = _import_maximum_seconds()

    result = MercariImportCSVResult.objects.filter(
        author=user, status=2).first()
    if result is None:
        logger.warn('事前に生成されているべきレコードが存在しません。 user=%s, status=2', user)
        return

    exclude_sellers = MercariExcludeSeller.objects.filter(author=user)
    try:
        # アイテムを一意にソートし、まだ未処理のアイテムのみをピックアップ
        items = MercariToAmazonItem.objects.filter(
            author=user, record_type=10, id__gte=latest_id).order_by('id')
        scraper = MercariItemScraper(RichsUtils.get_ip_address(user))
        ...
        for item in items:
            ...
            latest_id = item.id
            try:
                url = 'https://item.mercari.com/jp/' + item.purchase_item_id + '/'
                ...
                if len(item_infos) == 0:
                    error_items.append(
                        'アイテム取得失敗(終了済): {}'.format(item.purchase_item_id))
                    continue
                ...
                price_str = item_infos[0]['price']
                sold = item_infos[0]['sold']
                item.current_purchase_price = item.purchase_price = int(
                    price_str)
                item.condition_type = RichsUtils.mercari_to_amazon_condition(
                    item_infos[0]['condition'])
                item.condition_note = item_infos[0]['condition']
                item.current_purchase_seller_id = item.purchaseo_seller_id = item_infos[
                    0]['seller']
                item.current_purchase_seller_id_name = item.purchaseo_seller_id_name = item_infos[
                    0]['seller_name']
                ...
                print(traceback.format_exc())
                error_items.append(
                    'メルカリ情報取得中にエラーが発生: {}'.format(item.purchase_item_id))

        # 途中経過を更新
        def _or(a, b):
            return a if a is not None else b

        result.error_mercari_items = _or(
            result.error_mercari_items, 0) + len(error_items)
        if result.error_mercari_items_txt in [None, '']:
            result.error_mercari_items_txt = '\n'.join(error_items)
        else:
            result.error_mercari_items_txt = '\n'.join(
                [result.error_mercari_items_txt] + error_items)

        if completed:
            # 全てが終わっている場合はステータスを更新して次のタスクへ移る
            result.status = 3
            result.result_message = '[{}] メルカリ情報検索終了'.format(
                timezone.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            result.save()
            q = asynchelpers.get_import_queue()
            q.enqueue(import_item_internal_asin_download_task, user=user)
        else:
            # まだ処理がある場合は継続実行
            result.result_message = '[{}] メルカリ情報検索終了'.format(
                timezone.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            result.save()
            q = asynchelpers.get_import_queue()
            q.enqueue(import_item_internal_mercari_task,
                      user=user, latest_id=latest_id)
...
def import_item_internal_asin_download_task(user, latest_id=-1):
    ''' ASIN情報を取得する継続タスク。
    ...
    try:
        ...
        def save_into_redis(matching):
            ...
            for key, value in matching.items():
                ...
                rkey = asynchelpers.rkey('M2A', 'ASIN', user.username, key)
                r.hmset(rkey, data)
        ...
        if complated:
            # 全てが終わっている場合はステータスを更新して次のタスクへ移る
            q = asynchelpers.get_import_queue()
            q.enqueue(import_item_internal_asin_update_task, user=user)

        else:
            # まだ処理がある場合は継続実行
            q = asynchelpers.get_import_queue()
            q.enqueue(import_item_internal_asin_download_task,
                      user=user, latest_id=latest_id)
...
def import_item_internal_asin_update_task(user, latest_id=-1):
    ''' ASIN情報を取得する継続タスク。
    ...
    try:
        ...
        for item in items:
            ...
            rkey = asynchelpers.rkey(
                'M2A', 'ASIN', user.username, item.external_product_id)
            ...
            info = {k.decode('utf8'): v.decode('utf8')
            ...
            try:
                ...
                item.record_type = 12
                item.main_image_url = RichsUtils.download_to_mercari_folder(
        ...
        if result.error_asins_text in [None, '']:
            ...
            result.error_asins_text = '\n'.join(
                [result.error_asins_text] + errors)
        ...
        if completed:
            ...
            result.save()
            q = asynchelpers.get_import_queue()
            q.enqueue(import_item_internal_sku_download_task, user=user)
        else:
            # まだ処理がある場合は継続実行
            ...
            result.save()
            q = asynchelpers.get_import_queue()
            q.enqueue(import_item_internal_asin_update_task,
                      user=user, latest_id=latest_id)
...
def import_item_internal_sku_download_task(user, latest_id=-1):
    ''' SKU情報を取得する継続タスク。
    ...
    try:
        ...
        def save_into_redis(prices):
            ...
            for sku, value in prices.items():
                ...
                rkey = asynchelpers.rkey('M2A', 'SKU', user.username, sku)
                r.hmset(rkey, data)
                r.expire(rkey, timedelta(days=1))
        ...
        if completed:
            # 全てが終わっている場合はステータスを更新して次のタスクへ移る
            q = asynchelpers.get_import_queue()
            q.enqueue(import_item_internal_sku_update_task, user=user)
        else:
            # まだ処理がある場合は継続実行
            q = asynchelpers.get_import_queue()
            q.enqueue(import_item_internal_sku_download_task,
                      user=user, latest_id=latest_id)
...
def import_item_internal_sku_update_task(user, latest_id=-1):
    ''' SKU情報を取得する継続タスク。
    ...
    try:
        ...
        for item in items:
            ...
            latest_id = item.id
            rkey = asynchelpers.rkey(
                'M2A', 'SKU', user.username, item.item_sku)
            raw_cache = r.hgetall(rkey)
            if raw_cache is None or len(raw_cache) == 0:
            ...
            try:
                ...
                if info.get('amount', '') != '':
                    ...
                    #item.current_purchase_quantity = item.purchase_quantity = 0
        ...
        if completed:
            ...
            result.save()
            q = asynchelpers.get_import_queue()
            q.enqueue(import_item_internal_finalize, user=user)
        else:
            # まだ処理がある場合は継続実行
            ...
            result.save()
            q = asynchelpers.get_import_queue()
            q.enqueue(import_item_internal_sku_update_task,
                      user=user, latest_id=latest_id)
...
def import_item_internal_finalize(user):
    ''' CSV取り込み処理の最終処理 '''

    result = MercariImportCSVResult.objects.filter(
        author=user, status=5).first()
    if result is None:
        logger.warn('事前に生成されているべきレコードが存在しません。 user=%s, status=5', user)
        return

    # 正常処理結果数 (保存が成功した後に消す）
    result.success = MercariToAmazonItem.objects.filter(
        author=user, record_type=13).count()
    result.save()

    # 登録商品を登録完了状態にする。
    MercariToAmazonItem.objects.filter(
        author=user, record_type=13).update(record_type=0)
    ...
    result_messages.append(
        'CSVフォーマットエラーの為削除:{0}件'.format(result.error_record_numbers))
    ...
    result_messages.append(
        '出品できない商品である為削除:{0}件'.format(result.error_mercari_items))
    if (result.error_mercari_items > 0):
        # 削除事由は件数を集約して表示
        lines = result.error_mercari_items_txt.replace('\r', '').split('\n')
        ...
        for reason in sorted(error_items.keys()):
            result_messages.append(
                '- {}: {}件'.format(reason, error_items[reason]))
        result_messages.append('')
    ...
    result_messages.append(
        'Amazonに登録されていないASINの為削除:{0}件'.format(result.error_asins))
    if (result.error_asins > 0):
        result_messages.append('-- Amazonから取得できなかったASIN一覧  --')
        result_messages.append(result.error_asins_text)
        result_messages.append('')

    result_messages.append(
        'Amazonに登録されていないSKUの為削除:{0}件'.format(result.error_skus))
    if (result.error_skus > 0):
        result_messages.append('-- Amazonから取得できなかったSKU一覧 --')
...
def _get_importing_status(request):
    ...
    try:
        result = MercariImportCSVResult.objects.get(
            Q(author=request.user), ~Q(status=0))
        ...
        if result.status == 5:
            ...
            if result.status > 1:
                progress_messages.append('メルカリ商品情報を取得中・・・')
            if result.status > 2:
    ...
    except MercariImportCSVResult.DoesNotExist:
        return {'exists': False}
...
def import_item(request):
    if (request.method == 'POST'):
        ...
        if ('import_file' in request.FILES):
            # 書き込みが行われていない場合のみ
            progress_count = MercariImportCSVResult.objects.filter(
                Q(author=request.user), ~Q(status=0)).count()
            if progress_count > 0:
                return HttpResponseRedirect("/mercari/import_item")

           # 処理開始をDBに書き込む
            result = MercariImportCSVResult()
            result.user_check = False
            result.author = request.user
            result.status = 1
            result.save()

            path = RichsUtils.handle_uploaded_tmp_file(
                request.FILES['import_file'], request.user)
            #io.TextIOWrapper(request.FILES['import_file'], encoding=encoding)
            print(path)

            # 商品取り込み遅延処理
            q = asynchelpers.get_import_queue()
            q.enqueue(import_item_entry, path, request.user)
    ...
    result_list = MercariImportCSVResult.objects.filter(
    ...
    return render(request, 'mercari/import_item.html', params)
...
def exclude_seller(request):
    params = {}
    if (request.method == 'POST'):
        form = MercariExcludeSellerForm(
            request.POST, instance=MercariExcludeSeller())
        form.instance.author = request.user
        params['success'] = False
        if form.is_valid():
            if MercariExcludeSeller.objects.filter(author=request.user, seller_id=form.instance.seller_id).count() == 0:
                form.save()
                # 登録成功メッセージ
                params['success'] = True
    ...
    params['form'] = MercariExcludeSellerForm()
    params['sellers'] = MercariExcludeSeller.objects.filter(
        author=request.user)
    return render(request, 'mercari/exclude_seller.html', params)
...
def delete_exclude_seller(request):
    ...
    if (request.method == 'POST'):
        ...
        if delete_ids:
            MercariExcludeSeller.objects.filter(id__in=delete_ids).delete()
...
# 検索URL構築
def to_search_url(form):
    url = RichsUtils.MERCARI_URL_KEY_SEARCH
    condition = to_condition_params(form)
    shipping_payer = to_shipping_payer_params(form)
    status = to_status_param(form)
    sort_order = ''
    if (form['sort_order'].value() != 'standard'):
        sort_order = 'sort_order=' + form['sort_order'].value() + '&'
    url = url.format(_keyword=urllib.parse.quote(form['keyword'].value(), safe='?'), _sort_order=sort_order, _price_min=form['price_min'].value(
    ), _price_max=form['price_max'].value(), _condition=condition, _shipping_payer=shipping_payer, _status=status)
    # print(url)
    return url
...
def to_params_string(form, lookup, params):
    tmp = ''
    for param in params:
        if (form[param].value() == True):
            tmp = tmp + '&' + lookup[param]
    return tmp

# コンディションのURLを構築


def to_condition_params(form):
    lookup = {'condition_all': 'condition_all=1',
              'condition_id_1': 'item_condition_id%5B1%5D=1',
              'condition_id_2': 'item_condition_id%5B2%5D=1',
              'condition_id_3': 'item_condition_id%5B3%5D=1',
              'condition_id_4': 'item_condition_id%5B4%5D=1',
              'condition_id_5': 'item_condition_id%5B5%5D=1',
              'condition_id_6': 'item_condition_id%5B6%5D=1'
              }
    params = ['condition_all', 'condition_id_1', 'condition_id_2',
              'condition_id_3', 'condition_id_4', 'condition_id_5', 'condition_id_6']
    return to_params_string(form, lookup, params)

# 配送料の負担


def to_shipping_payer_params(form):
    lookup = {'shipping_payer_all': 'shipping_payer_all=1',
              'shipping_payer_id_1': 'shipping_payer_id%5B1%5D=1',
              'shipping_payer_id_2': 'shipping_payer_id%5B2%5D=1',
              }
    params = ['shipping_payer_all',
              'shipping_payer_id_1', 'shipping_payer_id_2']
    return to_params_string(form, lookup, params)

# 販売状況


def to_status_param(form):
    lookup = {'status_all': 'status_all=1',
              'status_id_on_sale': 'status_on_sale=1',
              'status_id_trading_sold_out': 'status_trading_sold_out=1'
              }
    params = ['status_all', 'status_id_on_sale', 'status_id_trading_sold_out']
    return to_params_string(form, lookup, params)
...
def research(request):
    ...
    if (request.method == 'POST'):
        ...
        form = MercariSearchForm(request.POST)
        params['form'] = form

        keyword = form['keyword'].value()
        is_banned, banned_item = RichsUtils.judge_banned_item(keyword)
        if is_banned:
            messages.info(
                request, '<{}>が含まれる商品の出品は禁止されています'.format(banned_item))
            return redirect('/mercari/research')
        ...
        if is_search:
            ...
            if search_type == '0':
                url = to_search_url(form)
                scraper = MercariSearchScraper(
                    RichsUtils.get_ip_address(request.user))
                items = scraper.get_products(url)
                set_items_for_params(params, items, 1, request.user,
                                     scraper.getNextPageURL(), scraper.getPrevPageURL())
            elif search_type == '1':
                url = 'https://item.mercari.com/jp/' + \
                    form['keyword'].value() + '/'
                scraper = MercariItemScraper(
                    RichsUtils.get_ip_address(request.user))
                items = scraper.get_products(url)
                set_items_for_params(params, items, 1, request.user)
        else:
            ...
            if is_next or is_prev:
                ...
                items = scraper.get_products(url)
                set_items_for_params(params, items, page, request.user,
                                     scraper.getNextPageURL(), scraper.getPrevPageURL())
        # END POST
    else:
        # GET
        params['form'] = MercariSearchForm(initial={
                                           'search_type': 0, 'condition_id_1': True, 'shipping_payer_id_2': True, 'status_id_on_sale': True})

    return render(request, 'mercari/research.html', params)


# 検索
@login_required
def research_amazon_by_mercari(request):
    ...
    if (request.method == 'POST'):
        ...
        scraper = MercariItemScraper(RichsUtils.get_ip_address(request.user))
        url = 'https://item.mercari.com/jp/' + item_id + '/'
        items = scraper.get_products(url)

        print(items)
        if (len(items) == 0 or items[0]['sold'] == 'True'):
            params['error_message'] = 'この商品は既に販売が終了しています。'
            return render(request, 'mercari/close.html', params)

        exclude_sellers = MercariExcludeSeller.objects.filter(
            author=request.user)
        if (RichsUtils.is_exclude_sellers(exclude_sellers, items[0]['seller']) == True):
            params['message'] = "注意"
            params['message_detail'] = "除外セラーの出品商品です。"

        params['item'] = items[0]
        keyword = items[0]['title']
        request.session['new_feed_item_info'] = items[0]

    log_context = 'research_mercari_by_amazon - {}'.format(
        request.user.username)
    amazon = AmazonScraper(RichsUtils.get_ip_address(
        request.user), context=log_context)
    # Amazon情報の検索
    ...
    amazon_items = amazon.get_products(url)

    ok_items = []

    if (len(amazon_items) > 0):
        asin_list = RichsUtils.amazon_items_to_asin_list(amazon_items)
        exclude_asins = ExcludeAsin.objects.filter(
            author=request.user, asin__in=asin_list)
    ...
    params['form'] = form
    return render(request, 'mercari/research_amazon_by_mercari.html', params)
...
def feed_amazon_new(request):
    ...
    def is_register():
        ...
        if is_banned:
            ...
            params['message_detail'] = '商品名に禁止ワード【{}】が含まれています'.format(
                banned_item)
            return False
        return True

    if is_register():
        if MercariToAmazonItem.objects.filter(author=request.user).count() >= request.user.max_items:
            params['error_message'] = '商品登録件数を超えているため、登録することができません。'
            return render(request, 'mercari/close.html', params)

        form = FeedNewForm(request.POST, instance=MercariToAmazonItem())

        if form.is_valid():

            count = MercariToAmazonItem.objects.filter(Q(author=request.user) & (
                Q(item_sku=form.instance.item_sku) | Q(current_purchase_item_id=form.instance.item_sku))).count()
            ...
            form.instance.current_purchase_seller_id_name = form.instance.purchaseo_seller_id_name
            ...
            form.instance.purchase_price = form.instance.current_purchase_price = int(
                request.POST.get('current_price'))
            ...
            # 画像の保存
            client = MercariItemScraper(
                RichsUtils.get_ip_address(request.user))

            # 画像の保存
            if (form.instance.main_image_url != None and form.instance.main_image_url != ''):
                if (form.instance.main_image_url.startswith('http')):
                    form.instance.main_image_url = RichsUtils.download_to_mercari_folder(
                        client, form.instance.main_image_url, request.user)
                else:
                    if ('main_image_file' in request.FILES):
                        f = request.FILES['main_image_file']
                        form.instance.main_image_url = RichsUtils.handle_uploaded_file_to_mercari_foler(
                            f, request.user)
            ...
            if (form.instance.other_image_url1 != None and form.instance.other_image_url1 != ''):
                if (form.instance.other_image_url1.startswith('http')):
                    form.instance.other_image_url1 = RichsUtils.download_to_mercari_folder(
                        client, form.instance.other_image_url1, request.user)
                else:
                    if ('other_image_file1' in request.FILES):
                        f = request.FILES['other_image_file1']
                        form.instance.other_image_url1 = RichsUtils.handle_uploaded_file_to_mercari_foler(
                            f, request.user)
            ...
            if (form.instance.other_image_url2 != None and form.instance.other_image_url2 != ''):
                if (form.instance.other_image_url2.startswith('http')):
                    form.instance.other_image_url2 = RichsUtils.download_to_mercari_folder(
                        client, form.instance.other_image_url2, request.user)
                else:
                    if ('other_image_file2' in request.FILES):
                        f = request.FILES['other_image_file2']
                        form.instance.other_image_url2 = RichsUtils.handle_uploaded_file_to_mercari_foler(
                            f, request.user)
            ...
            if (form.instance.other_image_url3 != None and form.instance.other_image_url3 != ''):
                if (form.instance.other_image_url3.startswith('http')):
                    form.instance.other_image_url3 = RichsUtils.download_to_mercari_folder(
                        client, form.instance.other_image_url3, request.user)
                else:
                    if ('other_image_file3' in request.FILES):
                        f = request.FILES['other_image_file3']
                        form.instance.other_image_url3 = RichsUtils.handle_uploaded_file_to_mercari_foler(
                            f, request.user)
        ...
        if request.POST:
            # 再入力の場合
            form = FeedNewForm(request.POST, instance=MercariToAmazonItem())
            params['images'] = request.session.get(
                'new_feed_item_info', {}).get('images', [])
        elif 'new_feed_item_info' in request.session:
            item_info = request.session['new_feed_item_info']

            count = MercariToAmazonItem.objects.filter(Q(author=request.user) & (
                Q(item_sku=item_info['item_id']) | Q(current_purchase_item_id=item_info['item_id']))).count()
            if (count > 0):
            ...
            # スクレイピングからForm
            initial_param['item_sku'] = item_info['item_id']
            ...
            initial_param['purchaseo_seller_id_name'] = item_info['seller_name']
            initial_param['quantity'] = 1

            price_setting = AmazonFeedPriceSettings.objects.get(
                author=request.user)

            try:
                initial_param['current_price'] = 0
                initial_param['current_price'] = value = int(
                    item_info['price'])
                standard_price = max(
            ...
            initial_param['condition_type'] = RichsUtils.mercari_to_amazon_condition(
                item_info['condition'])
        ...
        # プルダウンの生成
        form.fields['manufacturer'].widget = forms.Select(choices=(
            (e.brand_name, e.brand_name) for e in AmazonBrand.objects.filter(author=request.user)))
        form.fields['category'].widget = forms.Select(choices=(
            (f.value, f.name) for f in AmazonParentCategoryUser.objects.filter(author=request.user)))
        category = AmazonParentCategoryUser.objects.filter(
            author=request.user).first()
        if category != None:
            form.fields['recommended_browse_nodes'].widget = forms.Select(choices=(
                (g.value, g.name) for g in settings_amazon_views.get_child_user_model_class(category.value).objects.filter(author=request.user)))

        params['form'] = form

        return render(request, 'mercari/feed_amazon_new.html', params)
...
def feed_amazon_offer(request):
    ...
    if (request.method == 'POST'):
        ...
        form = FeedOfferForm(request.POST, instance=MercariToAmazonItem())
        if form.is_valid():
            ...
            form.instance.author = request.user
            form.instance.record_type = 0
            form.instance.item_name = item_info['title']
            form.instance.current_purchase_seller_id = form.instance.purchaseo_seller_id = item_info[
                'seller']
            form.instance.current_purchase_seller_id_name = form.instance.purchaseo_seller_id_name = item_info[
                'seller_name']
            form.instance.purchase_item_id = form.instance.current_purchase_item_id = form.instance.item_sku
            form.instance.purchase_quantity = form.instance.current_purchase_quantity = form.instance.quantity
            form.instance.purchase_fulfillment_latency = form.instance.current_purchase_fulfillment_latency = form.instance.fulfillment_latency
        ...
        if 'new_feed_item_info' in request.session:
            ...
            try:
                initial_param['current_price'] = 0
                initial_param['current_price'] = int(item_info['price'])
                initial_param['standard_price'] = int(
                    int(item_info['price']) * price_setting.margin_offer + 0.5)
            except Exception as e:
                print(e)
                pass
        ...
        form.fields['external_product_id_type'].widget = forms.Select(
            choices=(('ASIN', 'ASIN'),))
        params['form'] = form

        return render(request, 'mercari/feed_amazon_offer.html', params)


# 新規CSVファイルを出力のメイン処理部分
def _export_amazon_new_csv(csv_format, m2a_items, user):
    format = csv_format
    list = m2a_items
    if len(list) <= 0:
        return
    url_base = settings.RICHS_PROTOCOL + '://' + settings.RICHS_FQDN + \
        settings.RICHS_URL_IMAGE + '/mercari/' + user.username + '/'
    ...
    csv_file_name = format.format + \
        str(datetime.now().timestamp()).replace('.', '') + '.csv'
    ...
    with codecs.open(csv_tpl, 'r', 'cp932') as fin, \
        ...
        for i, e in enumerate(list):
            ...
            if (format.recommended_browse_nodes > -1 and RichsUtils.is_valid_str(e.recommended_browse_nodes)):
                cols[format.recommended_browse_nodes] = '"' + \
                    e.recommended_browse_nodes + '"'
            ...
            if (format.main_image_url > -1 and RichsUtils.is_valid_str(e.main_image_url)):
                cols[format.main_image_url] = '"' + \
                    url_base + e.main_image_url + '"'
            ...
            if (format.product_description > -1 and RichsUtils.is_valid_str(e.product_description)):
                cols[format.product_description] = '"' + \
            ...
            if (format.other_image_url1 > -1 and RichsUtils.is_valid_str(e.other_image_url1)):
                cols[format.other_image_url1] = '"' + \
                    url_base + e.other_image_url1 + '"'
            if (format.other_image_url2 > -1 and RichsUtils.is_valid_str(e.other_image_url2)):
                cols[format.other_image_url2] = '"' + \
                    url_base + e.other_image_url2 + '"'
            if (format.other_image_url3 > -1 and RichsUtils.is_valid_str(e.other_image_url3)):
                cols[format.other_image_url3] = '"' + \
                    url_base + e.other_image_url3 + '"'
            ...
            if (format.standard_price_points > -1 and e.standard_price_points != None):
                cols[format.standard_price_points] = str(
                    e.standard_price_points)
    ...
    # DB書き込み
    MercariToAmazonCSV.objects.create(
        file_name=csv_file_name, feed_type=0, author=user)
...
def export_amazon_new_csv(request):
    ...
    if (request.method == 'POST'):
        ...
        if not do_post:
            ...
            for format in CSVFormat.objects.all():
                ...
                if output_all:
                    list = MercariToAmazonItem.objects.filter(format=format.format,
                                                              author=request.user, feed_type=0, csv_flag=0)
                else:
                    ids = ids if ids else []
                    list = MercariToAmazonItem.objects.filter(id__in=ids, format=format.format,
                                                              author=request.user, feed_type=0, csv_flag=0)
                if len(list) > 0:
    ...
    # GET/POST共通
    items = MercariToAmazonItem.objects.filter(
        author=request.user, csv_flag=0, feed_type=0)
    for i, item in enumerate(items):
        item.local_id = i + 1
    params['items'] = items
    params['url_item'] = settings.RICHS_PROTOCOL + '://' + settings.RICHS_FQDN + \
        settings.RICHS_URL_IMAGE + '/mercari/' + request.user.username
    return render(request, 'mercari/export_amazon_new_csv.html', params)
...
def export_amazon_offer_csv_internal(list, user):
    ...
    if len(list) > 0:
        ...
        csv_file_name = str(datetime.now().timestamp()
                            ).replace('.', '') + '.csv'
        csv_file_path = out_folder + '/' + csv_file_name
        ...
        for i, e in enumerate(list):
            try:
                ...
                cols[2] = str(
                    e.standard_price_points) if e.standard_price_points != None else ''
        ...
        # DB書き込み
        MercariToAmazonCSV.objects.create(
            file_name=csv_file_name, feed_type=1, author=user)
...
def export_amazon_offer_csv(request):
    ...
    if (request.method == 'POST'):
        ...
        if not do_post:
            ...
            output_all = 'output_all' in request.POST
            if output_all:
                list = MercariToAmazonItem.objects.filter(
                    author=request.user, feed_type=1, csv_flag=0)
            else:
                ids = ids if ids else []
                list = MercariToAmazonItem.objects.filter(id__in=ids,
                                                          author=request.user, feed_type=1, csv_flag=0)
            if len(list) > 0:
    ...
    # GET/POST共通
    items = MercariToAmazonItem.objects.filter(
        author=request.user, csv_flag=0, feed_type=1)
    for i, item in enumerate(items):
        item.local_id = i + 1
    params['items'] = items
    params['url_item'] = settings.RICHS_PROTOCOL + '://' + settings.RICHS_FQDN + \
        settings.RICHS_URL_IMAGE + '/mercari/' + request.user.username
    return render(request, 'mercari/export_amazon_offer_csv.html', params)
...
def _get_latest_item2csv(user):
    ...
    try:
        ...
        max_item_per_day = OverrideConstantValue.get_value(
            'EXPORT_CANDIDATE_ITEM_PER_DAY', 0, int)
        item2csv = ItemCandidateToCsv.objects.create(
            owner=user, max_output=max_item_per_day)
...
def _convert_candidate_to_formal_item(candidates):
    ...
    for candidate in candidates:
        ...
        if c > 0:
            ...
            continue
        item = MercariToAmazonItem.objects.create(
            ...
            purchaseo_seller_id_name=candidate.purchaseo_seller_id_name,
            ...
            current_purchase_seller_id_name=candidate.current_purchase_seller_id_name,
...
def _export_amazon_new_csv_from_candidate(user, ids=None, item2csv=None):
    ...
    if ids is not None:
        candidates = MercariToAmazonItemCandidate.objects.filter(
...
def _export_amazon_new_csv_from_candidate_post(request, params):
    ...
    if is_delete:
        if not ids:
            return
        targets = MercariToAmazonItemCandidate.objects.filter(id__in=ids)
    ...
    (succ, msg, detail) = _export_amazon_new_csv_from_candidate(
        request.user, ids, item2csv)
    params['success'] = succ
    params['message'] = msg
...
def export_amazon_new_csv_from_candidate(request):
    ...
    # GET/POST共通
    items = MercariToAmazonItemCandidate.objects.filter(
    ...
    params['url_item'] = settings.RICHS_PROTOCOL + '://' + settings.RICHS_FQDN + \
        settings.RICHS_URL_IMAGE + '/mercari/' + request.user.username
    return render(request, 'mercari/export_amazon_new_csv_from_candidate.html', params)
...
def download_amazon_csv(request):
    ...
    new_feed_csv = MercariToAmazonCSV.objects.filter(
        author=request.user, feed_type=0).order_by('-created_date')
    for i, csv in enumerate(new_feed_csv):
        csv.local_id = i + 1

    offer_feed_csv = MercariToAmazonCSV.objects.filter(
        author=request.user, feed_type=1).order_by('-created_date')
    for i, csv in enumerate(offer_feed_csv):
        csv.local_id = i + 1

    params['image_base'] = settings.RICHS_PROTOCOL + '://' + \
        settings.RICHS_FQDN + '/output/' + request.user.username
    params['new_feed_csv'] = new_feed_csv
    params['offer_feed_csv'] = offer_feed_csv

    return render(request, 'mercari/download_amazon_csv.html', params)
...
def edit_amazon_new(request):
    ...
    if (request.method == 'POST'):
        ...
        if form.is_valid():
            try:
                e = MercariToAmazonItem.objects.get(
                    author=request.user, item_sku=form.instance.item_sku)
                ...
                # 画像の保存
                client = MercariSearchScraper(
                    RichsUtils.get_ip_address(request.user))

                # 画像の保存
                if (form.instance.main_image_url != None and form.instance.main_image_url != ''):
                    if (form.instance.main_image_url.startswith('http')):
                        e.main_image_url = RichsUtils.download_to_mercari_folder(
                            client, form.instance.main_image_url, request.user)
                    else:
                        if ('main_image_file' in request.FILES):
                            f = request.FILES['main_image_file']
                            e.main_image_url = RichsUtils.handle_uploaded_file_to_mercari_foler(
                                f, request.user)
                ...
                if (form.instance.other_image_url1 != None and form.instance.other_image_url1 != ''):
                    if (form.instance.other_image_url1.startswith('http')):
                        e.other_image_url1 = RichsUtils.download_to_mercari_folder(
                            client, form.instance.other_image_url1, request.user)
                    else:
                        if ('other_image_file1' in request.FILES):
                            f = request.FILES['other_image_file1']
                            e.other_image_url1 = RichsUtils.handle_uploaded_file_to_mercari_foler(
                                f, request.user)
                ...
                if (form.instance.other_image_url2 != None and form.instance.other_image_url2 != ''):
                    if (form.instance.other_image_url2.startswith('http')):
                        e.other_image_url2 = RichsUtils.download_to_mercari_folder(
                            client, form.instance.other_image_url2, request.user)
                    else:
                        if ('other_image_file2' in request.FILES):
                            f = request.FILES['other_image_file2']
                            e.other_image_url2 = RichsUtils.handle_uploaded_file_to_mercari_foler(
                                f, request.user)
                ...
                if (form.instance.other_image_url3 != None and form.instance.other_image_url3 != ''):
                    if (form.instance.other_image_url3.startswith('http')):
                        e.other_image_url3 = RichsUtils.download_to_mercari_folder(
                            client, form.instance.other_image_url3, request.user)
                    else:
                        if ('other_image_file3' in request.FILES):
                            f = request.FILES['other_image_file3']
                            e.other_image_url3 = RichsUtils.handle_uploaded_file_to_mercari_foler(
                                f, request.user)
            ...
            except MercariToAmazonItem.DoesNotExist:
        ...
        try:
            e = MercariToAmazonItem.objects.get(
                author=request.user, item_sku=item_sku)
            ...
            params['base_url'] = RichsUtils.get_mercari_image_base_url(
                request.user)
        ...
        return render(request, 'mercari/edit_amazon_new.html', params)
...
def edit_amazon_new_candidate(request):
    ...
    if (request.method == 'POST'):
        form = FeedNewForm(
            request.POST, instance=MercariToAmazonItemCandidate())
        if form.is_valid():
            try:
                e = MercariToAmazonItemCandidate.objects.get(
                    author=request.user, item_sku=form.instance.item_sku)
            ...
            except MercariToAmazonItemCandidate.DoesNotExist:
                params['error_message'] = '該当データは、存在しません。'
                return render(request, 'mercari/close.html', params)
        ...
        return render(request, 'mercari/edit_amazon_new_candidate.html', params)
...
def edit_amazon_offer(request):
    ...
    if (request.method == 'POST'):
        ...
        item_sku = request.GET.get('item_sku')
        form = FeedOfferForm(instance=MercariToAmazonItem.objects.get(
            author=request.user, item_sku=item_sku))
        form.fields['external_product_id_type'].widget = forms.Select(
            choices=(('ASIN', 'ASIN'),))
        params['form'] = form
        return render(request, 'mercari/edit_amazon_offer.html', params)


# 内部処理(検索フォーム)
def amazon_offer_research_search_view(request):
    params = {}
    params['form'] = AmazonOfferMercariSearchForm(initial={
        'search_type': 0, 'rateing': 90.0, 'similarity': 0.80,
        'condition_id_1': True, 'shipping_payer_id_2': True,
        'status_id_on_sale': True, 'is_export_csv': '1'
    })
    return render(request, 'mercari/amazon_offer_research.html', params)
...
def amazon_offer_research_loading_view(request, e, user, params=None):
    ...
    if URLSkipRequest.objects.filter(view=21, author=request.user).count() > 0:
        # スキップ要求中
        e.status = 8

    entityList = StopRequest.objects.filter(view=21,  author=user)
    if (len(entityList) > 0):
        # 停止要求中
        e.status = 9
    ...
    e.duration = RichsUtils.timestamp_duration_to_display_string(
        e.created_date, e.updated_date)

    params['data'] = e
    return render(request, 'mercari/watch_offer_reserch_transaction.html', params)
...
def _add_amazon_search_url(request):
    ...
    watch = OfferReserchWatcher.objects.get(
        author=request.user, research_type=1)
    if watch.status != 0:
        params = {}
        params['success'] = False
...
def _interrupt_amazon_search_url(request):
    ...
    URLSkipRequest.objects.create(view = 21,  author = request.user)
    e = OfferReserchWatcher.objects.get(author=request.user, research_type=1)
...
def amazon_offer_research_post(request):
    ...
    if ('close' in request.POST):
        ...
        StopRequest.objects.filter(view=21, author=request.user).delete()
        URLSkipRequest.objects.filter(view=21, author=request.user).delete()
        OfferReserchWatcher.objects.filter(
            author=request.user, research_type=1).delete()
        return HttpResponseRedirect("/mercari/amazon_offer_research")

    # URLスキップ要求時はDB上にスキップ依頼を発行
    if ('skip' in request.POST):
        URLSkipRequest.objects.create(view=21,  author=request.user)
        e = OfferReserchWatcher.objects.get(
            author=request.user, research_type=1)
    ...
    if ('stop' in request.POST):
        print('停止ボタン')
        stop = StopRequest(view=21,  author=request.user)
        stop.save()
        e = OfferReserchWatcher.objects.get(
            author=request.user, research_type=1)
        return amazon_offer_research_loading_view(request, e, request.user)
    ...
    if MercariToAmazonItem.objects.filter(author=request.user).count() >= request.user.max_items:
        ...
        e = OfferReserchWatcher(research_type=1, status=1, total=0, exclude_asin=0,
    ...
    # 通常の検索処理
    form = AmazonOfferMercariSearchForm(request.POST)
    search_type = form['search_type'].value()
    keyword = form['keyword'].value()
    target_urls = [
        AmazonSearchUtils.keyword_to_url(search_type, keyword),
        AmazonSearchUtils.keyword_to_url(
            search_type, form['extra_keyword1'].value()),
        AmazonSearchUtils.keyword_to_url(
            search_type, form['extra_keyword2'].value()),
        AmazonSearchUtils.keyword_to_url(
            search_type, form['extra_keyword3'].value()),
        AmazonSearchUtils.keyword_to_url(
            search_type, form['extra_keyword4'].value()),
        AmazonSearchUtils.keyword_to_url(
            search_type, form['extra_keyword5'].value()),
        AmazonSearchUtils.keyword_to_url(
            search_type, form['extra_keyword6'].value()),
        AmazonSearchUtils.keyword_to_url(
            search_type, form['extra_keyword7'].value()),
        AmazonSearchUtils.keyword_to_url(
            search_type, form['extra_keyword8'].value()),
        AmazonSearchUtils.keyword_to_url(
            search_type, form['extra_keyword9'].value()),
    ]
    ...
    entityList = OfferReserchWatcher.objects.filter(
        author=request.user, research_type=1)
    ...
    # 以降の処理では AmazonOfferMercariSearchForm のキーが必要
...
def amazon_offer_research_progress(request):
    ...
    if res['status'] == 0:
        ...
        if StopRequest.objects.filter(view=21, author=request.user).count() > 0:
            # 停止要求中
            res['status'] = 9
    ...
    res['start_time'] = RichsUtils.timestamp_to_display_string(
        watch.created_date)
    res['end_time'] = RichsUtils.timestamp_to_display_string(watch.end_date)
    res['duration'] = RichsUtils.timestamp_duration_to_display_string(
        watch.created_date, watch.updated_date)
...
def _ride_search_config():
    try:
        # メルカリ特有の条件判断
        return settings.MERCARI_RIDE_SEARCH_CONFIG
    except:
        # 設定がない場合
        return {}
...
# データを保存
def available_for_purchase(amazon_item, mercari_item, value, user, price_setting, amazon_image):
    # 購入可能なアイテムと判断された場合は 0 を返す
    try:
        # 商品数超過
        if MercariToAmazonItem.objects.filter(author=user).count() >= user.max_items:
            return -3

        sku = mercari_item['item_id']
        c = MercariToAmazonItem.objects.filter(
        ...
        # メルカリのコンディションを取得
        condition_type = RichsUtils.mercari_to_amazon_condition(
            mercari_item['condition'])
        if (condition_type == 'ERR'):
            # コンディション不一致
            return -1

        # メルカリの金額取得
        m_price = int(mercari_item['price'])
        ...
        if condition_type == 'New':
            ...
            # 新品がなければ、中古の価格で
            elif (amazon_item['price_old'] != '' and amazon_item['price_old'] != '-1'):
                print('B')
            ...
            if (amazon_item['price_old'] != '' and amazon_item['price_old'] != '-1'):
                a_price = int(amazon_item['price_old'])
            # 中古がなければ、新品の価格で
            elif (amazon_item['price_new'] != '' and amazon_item['price_new'] != '-1'):
                a_price = int(amazon_item['price_new'])
        ...
        if standard_price <= 0:
            return -1
        if price_setting.margin_offer_percent_url > 0:
            diff_percent = 100 * diff / standard_price
            if diff_percent < price_setting.margin_offer_percent_url:
                # 最低利益率を下回っている
                return - 1
        ...
        # タイトルが短すぎる場合
        if len(mercari_item['title']) < conf.get('MINIMUM_TITLE_LENGTH', 0):
            return -4
        ...
        # 特定の発送元からは仕入れを行わない
        if mercari_item['delivery_from'] in conf.get('IGNORE_DELIVERY_FROM', []):
            return -4
        # 発送が指定日時以上かかる場合
        if RichsUtils.str2int(mercari_item['fulfillment_latency'], -1) > conf.get('MAXIMUM_FULFILLMENT'):
            return -4
...
# データを保存
def store(amazon_item, mercari_item, value, user, price_setting, amazon_default_setting, amazon_image):
    ...
    try:
        ...
        sku = mercari_item['item_id']
        c = MercariToAmazonItem.objects.filter((Q(item_sku=sku) | Q(
            current_purchase_item_id=sku)) & Q(author=user)).count()
        if (c > 0):
            print('重複')
            return -2
        ...
        if condition_type == 'New':
            ...
            # 中古の場合はメルカリのコンディションを記載
            e.condition_note = mercari_item['condition']
        ...
        e.current_purchase_seller_id = e.purchaseo_seller_id = mercari_item['seller']
        e.current_purchase_seller_id_name = e.purchaseo_seller_id_name = mercari_item[
            'item_id']
        e.purchase_item_id = e.current_purchase_item_id = mercari_item['item_id']
        e.purchase_quantity = e.current_purchase_quantity = e.quantity = 1
        e.csv_flag = 0
        e.purchase_price = e.current_purchase_price = m_price
        e.main_image_url = RichsUtils.copy_image_to_mercari_folder(
            amazon_image, user)
        e.author = user
...
def create_item_candidate_in_ride_search(amazon_item, mercari_item,
    ...
    try:
        ...
        c = MercariToAmazonItemCandidate.objects.filter(
            (Q(item_sku=sku) | Q(current_purchase_item_id=sku)) & Q(author=user)).count()
        if (c > 0):
            return None
        c = MercariToAmazonItem.objects.filter(
        ...
        c = MercariToAmazonItemCandidate.objects.filter(author=user,
                                                        external_product_id_type='ASIN', external_product_id=amazon_item['asin']).count()
        if (c > 0):
            return None
        c = MercariToAmazonItem.objects.filter(author=user,
                                               external_product_id_type='ASIN', external_product_id=amazon_item['asin']).count()
        if (c > 0):
            return None
        ...
        price_settings = AmazonFeedPriceSettings.objects.filter(
            author=user).first()
        minimum = price_settings.default_minimum_item_price if price_settings is not None else 3000
        standard_price = max(
            int(m_price * price_settings.margin_new + 0.5), minimum)

        # 情報を自動入力
        e = MercariToAmazonItemCandidate()
        ...
        e.item_sku = sku
        initial_item_name = ItemTitleConverter.convert(mercari_item['title'])
        ...
        # カテゴリ(詳細)
        nodesmodel = settings_amazon_views.get_child_user_model_class(
            e.category)
        ...
        fmt = fmtmodel.objects.filter(
            author=user, value=e.recommended_browse_nodes).first()
        if fmt is not None:
            e.format = fmt.format
        ...
        image_urls = [url for url in mercari_item['images']]
        with ImageDownloader(image_base, image_urls) as downloader:
            if len(image_urls) > 0:
                # 画像の保存(あるだけ)
                e.main_image_url = RichsUtils.copy_image_to_mercari_folder(
                    downloader.get(image_urls[0]), user)
                if len(image_urls) > 1:
                    e.other_image_url1 = RichsUtils.copy_image_to_mercari_folder(
                        downloader.get(image_urls[1]), user)
                if len(image_urls) > 2:
                    e.other_image_url2 = RichsUtils.copy_image_to_mercari_folder(
                        downloader.get(image_urls[2]), user)
                if len(image_urls) > 3:
                    e.other_image_url3 = RichsUtils.copy_image_to_mercari_folder(
                        downloader.get(image_urls[3]), user)
            else:
...
#
#  一気通貫用のメルカリリサーチ用URL生成
#
def mercari_url_foramazon_offer_research(params, keyword, price_min, price_max):
    new_form = AmazonOfferMercariSearchForm(initial={
        'search_type': 0, 'keyword': keyword,
        'price_min': price_min, 'price_max': price_max,
        'condition_all': params.get('condition_all'),
        'condition_id_1': params.get('condition_id_1'),
        'condition_id_2': params.get('condition_id_2'),
        'condition_id_3': params.get('condition_id_3'),
        'condition_id_4': params.get('condition_id_4'),
        'condition_id_5': params.get('condition_id_5'),
        'condition_id_6': params.get('condition_id_6'),
        'shipping_payer_all':  params.get('shipping_payer_all'),
        'shipping_payer_id_1': params.get('shipping_payer_id_1'),
        'shipping_payer_id_2': params.get('shipping_payer_id_2'),
        'status_all': params.get('status_all'),
        'status_id_on_sale': params.get('status_id_on_sale'),
        'status_id_trading_sold_out': params.get('status_id_trading_sold_out'),
        'sort_order': params.get('sort_order')
    })
    return to_search_url(new_form)
...
def _get_offer_research_scrapers(user):
    ...
    context = 'MercariBGSearch:{}'.format(user.username)
    amazon_scraper = AmazonScraper(ipaddr, context=context)
    mercari_scraper = MercariSearchScraper(ipaddr)
    mercari_item_scraper = MercariItemScraper(ipaddr)
    ...
    return (amazon_scraper, mercari_scraper, mercari_item_scraper)
...
def amazon_offer_research_entry(params, url, user, trial=0, rest_wait_count=0):
    ...
    try:
        try:
            watch = OfferReserchWatcher.objects.get(
                research_type=1, author=user)
        except OfferReserchWatcher.DoesNotExist:
            logger.warn('相乗り商品抽出レコードが未登録です。 research_type=1, author=%s', user)
            return
        ...
        if len(entityList) > 0:
            entityList.delete()
            OfferReserchWatcher.objects.get(
                research_type=1, author=user).delete()
        ...
        # API情報の登録がない場合は処理を中断
        (amazon_scraper, _m, _mi) = _get_offer_research_scrapers(user)
        if amazon_scraper is None:
            logger.info('MWS APIの情報登録がありません。 research_type=1, author=%s', user)
            watch.delete()
            return
        ...
        pools = dict(
            ...
            exclude_sellers=MercariExcludeSeller.objects.filter(author=user),
            # Amazon出品デフォルト設定
            amazon_default_setting=AmazonDefaultSettings.objects.get(
                author=user),
        ...
        try:
            # Amazon の情報を取得
            amazon_items = amazon_scraper.get_products(
                url, raise_when_5xx=True)
        except Exception:
            ...
            logger.info('scraping failed and enqueue retry: {} user for {} trial(s)'.format(
                user, trial+1))
            return
        ...
        try:
            ...
            deletes = MercariToAmazonItemCandidate.objects.filter(
                author=user, created_date__lte=expired)
...
def amazon_offer_research_task(params, url, user, pools, trial=0, rest_wait_count=0):
    ...
    try:
        ...
        skipRequests = URLSkipRequest.objects.filter(view=21, author=user)
        ...
        logger.debug(
            '[seq=%s] started amazon_offer_research_task', pools['sequence'])

        amazon_items = pools['rest_amazon_items']

        watch = OfferReserchWatcher.objects.get(research_type=1, author=user)
        ...
        if rest_wait_count > 0:
            ...
            queue = asynchelpers.get_queue()
            queue.enqueue(amazon_offer_research_task,
                          kwargs=dict(params=params, url=url, user=user, pools=pools, trial=trial, rest_wait_count=rest_wait_count-1))
            return
        ...
        for amazon_item in amazon_items:
            ...
            try:
                ...
                # 1Amazon Itemの検索を実施
                amazon_offer_research_for_item(
                    params, user, pools, watch, amazon_item)

                # 検索結果の更新
                if watch.feed_item > prev_feed or watch.new_feed_item_candidate > prev_candidate:
                    pools['item_search_found'] = pools.get(
                        'item_search_found', 0) + 1
            except Exception as e:
                logger.exception(e)
            ...
            # 指定時間が経過している場合、作業を中断
            delta = timedelta(
                seconds=settings.ASYNC_WORKER['TASK_RECOMMENDED_MAXIMUM_SECONDS'])
        ...
        if len(rest_amazon_items) <= 0:
            ...
            try:
                next_amazon_items = amazon_scraper.get_products(
                    next_url, raise_when_5xx=True)
            except Exception as err:
                # 5xxで取得できない場合, あるいはAmazonへの通信で不明なエラーが発生した場合はリトライをかける
                retry_settings = _amazon_retry_settings()
                ...
                _save_background_search_progress(watch, current_search)
                queue.enqueue(amazon_offer_research_task,
                              kwargs=dict(params=params, url=url, user=user, pools=pools, trial=trial+1, rest_wait_count=rest_wait_count))
                logger.info('search scraping failed and enqueue retry: {} user for {} trial(s)'.format(
                    user, trial+1))
                return
...
def amazon_offer_research_for_item(params, user, pools, watch, amazon_item):
    ...
    (amazon_scraper, mercari_scraper,
     mercari_item_scraper) = _get_offer_research_scrapers(user)
    ...
    # 検索条件に利益額を入れる。
    margin = pools['price_setting'].margin_offer_url + \
        pools['price_setting'].offset_offer_price_url
    ...
    mercari_url = mercari_url_foramazon_offer_research(
        params, keyword, price_min, price_max)
    mercari_items = mercari_scraper.get_products(mercari_url)
    ...
    banned_list = RichsUtils.get_banned_list()
    for chunk in RichsUtils.chunkof(mercari_items, 10):
        if detected_similar_item:
            break

        valid_items = []
        for mercari_item in chunk:
            # 禁止ワード判定
            (is_banned, banned_keyword) = RichsUtils.judge_banned_item(
                mercari_item['title'], banned_list)
            if not is_banned:
                valid_items.append(mercari_item)

        image_urls = [c['images'][0] for c in valid_items]
        with ImageDownloader(pools['image_base'], image_urls) as downloader:
            for mercari_item in valid_items:
                # 比較のためにローカルに保存
                mercari_item_id = mercari_item['item_id']
                mercari_image_url = mercari_item['images'][0]
                mercari_image = downloader.get(mercari_image_url)
                if mercari_image is None:
                    # ダウンロード失敗
                    logger.debug('failed to download: %s', mercari_image_url)
                    continue
                ...
                (detected_similar_item, value) = ItemImageComparator.similar_fast(
                    amazon_image, mercari_image, float(params.get('similarity')))
                ...
                # 禁止セラー判定
                item_url = 'https://item.mercari.com/jp/' + mercari_item_id + '/'
                feed_item = mercari_item_scraper.get_products(item_url)
                seller_id = feed_item[0]['seller']
                ...
                if result == -3:
                    ...
                    # その他例外が発生したため無視して次のアイテムを検索
                    detected_similar_item = False
                    continue

                # 最終的な画像判定を行う
                (final_image_matched, _) = ItemImageComparator.similar(
                    amazon_image, mercari_image)
    ...
    if not detected_similar_item and len(detected_similar_item_candidates) > 0:
        ...
        for never_used_candidate in detected_similar_item_candidates:
            RichsUtils.delete_file_if_exist(
                never_used_candidate.main_image_url)

    elif not detected_similar_item:
        # Mercariアイテム全てが不一致である場合
        watch.condition_different += 1
...
def _amazon_offer_research_finalize_for_single(params, user, watch):
    ...
    if params.get('is_export_csv') == '1':
        try:
            if watch.feed_item > 0:
                watch.status = 2
                m2a_items = MercariToAmazonItem.objects.filter(
                    feed_type=1, csv_flag=0, author=user)
                if len(m2a_items) > 0:
                    export_amazon_offer_csv_internal(m2a_items, user)
    ...
    # 停止要求ファイルを全て削除する。
    StopRequest.objects.filter(view=21, author=user).delete()
